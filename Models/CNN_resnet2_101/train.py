...
from models import custom_resnet101, custom_resnet50
...
if config['model_type'] == 'resnet50':
    model = custom_resnet50(num_classes=7).to(device)
elif config['model_type'] == 'resnet101':
    model = custom_resnet101(num_classes=7).to(device)
else:
    raise ValueError("Invalid model_type in config. Choose 'resnet50', or 'resnet101'.")
